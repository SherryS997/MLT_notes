...
2. **When the covariance matrices are Identity matrices for both classes**: The decision boundary is linear as well as the perpendicular bisector of the line drawn from $\hat{\mu}_1$ and $\hat{\mu}_0$.
...
\end{align*}
$$
x_{test}^T(\hat{\Sigma}_1^{-1}-\hat{\Sigma}_0^{-1})x-2(\hat{\mu}_1^T\hat{\Sigma}_1^{-1}-\hat{\mu}_0^T\hat{\Sigma}_0^{-1})x+(\hat{\mu}_0^T\hat{\Sigma}_0^{-1}\hat{\mu}_0-\hat{\mu}_1^T\hat{\Sigma}_1^{-1}\hat{\mu}_1) + log(\frac{1-\hat{p}}{\hat{p}}) \ge 0
$$
